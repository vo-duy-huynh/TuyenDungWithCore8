    ...
    function AppViewModel() {
        ...
        self.sendToRoom = function (room, message) {
            if (room.name()?.length > 0 && message.length > 0) {
                fetch('/api/Messages', {
                    ...
                    body: JSON.stringify({ room: room.name(), content: message, fromFullName: "huynh", fromUserName: "huynh", avatar: "huynh" })
                });
            }
        }
        ...
        self.joinRoom = function (room) {
            connection.invoke("Join", room.name()).then(function () {
                ...
                self.userList();
                self.messageHistory();
                document.querySelector("input[name='invite_link']").value = room.name();
            });
        }
        ...
        self.createRoom = function () {
            ...
            fetch('/api/Rooms/', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ name: roomName, admin: "huynh" })
            });
        }
        ...
        self.editRoom = function () {
            ...
            fetch('/api/Rooms/' + roomId, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ id: roomId, name: roomName, admin: "huynh" })
            });
        }
